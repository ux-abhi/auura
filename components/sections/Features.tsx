...
// Custom SVG icons — stroke only, no fill
function IconHRV() {
  return (
...
const FEATURES = [
  {
    ...
    name: 'HRV Sensing',
    description:
      'Optical PPG at the sternum reads heart rate variability — your body\'s clearest stress signal.',
    tag: 'Cardiac',
  },
  {
    Icon: IconEDA,
    name: 'EDA Monitoring',
    description:
      'Electrodermal activity tracks skin conductance changes that precede conscious emotion.',
    tag: 'Neural',
  },
  {
    Icon: IconChip,
    ...
    description:
      'A 400KB INT8 model runs inference locally. No cloud. No latency. No exposure.',
    tag: 'Privacy',
  },
  {
    Icon: IconCircumplex,
    ...
    description:
      'Mood is plotted on valence × arousal axes. Continuous, contextual, never categorical.',
    tag: 'Affective AI',
  },
  {
    Icon: IconBluetooth,
    name: 'BLE Ecosystem',
    description:
      'Connects to smart lights, companion app, and optional EEG earbud. Mood preset only — never raw data.',
    tag: 'Connectivity',
  },
  {
    Icon: IconPendant,
    name: 'Jewellery Form',
    description:
      'Stainless steel and glass. 18g. Indistinguishable from a pendant necklace. Worn 24/7.',
    tag: 'Design',
  },
]
    ...
    <section id="features" data-section="features" className="bg-white py-[120px] px-6">
      <div className="mx-auto" style={{ maxWidth: 980 }}>
        ...
        >
          <motion.p
            variants={fadeIn}
            className="text-[11px] tracking-[0.18em] uppercase text-aurra-mid mb-4"
          >
            What&apos;s inside
          </motion.p>
          <motion.h2
          ...
          >
            ...
            <em className="not-italic italic">Designed for the soul.</em>
          </motion.h2>
        </motion.div>
        ...
        >
          {FEATURES.map((feature) => (
            <motion.div
              key={feature.name}
              variants={scaleIn}
              className="feature-card border border-aurra-light rounded-card p-8"
            >
              <feature.Icon />
              <h3 className="font-display text-[20px] font-normal text-aurra-dark mt-5 mb-2 leading-snug">
                {feature.name}
              </h3>
              <p className="text-[14px] text-aurra-mid leading-[1.65] mb-4">
                {feature.description}
              </p>
              <span
                className="inline-block text-[10px] tracking-[0.12em] uppercase text-aurra-mid border border-aurra-light"
